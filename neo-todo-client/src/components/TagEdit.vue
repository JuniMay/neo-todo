...
import { fetchDurationTask, fetchReminderTask, fetchTags, fetchAllTags, addTag, deleteTag, addTaskTag, deleteTaskTag } from '../utils';
    ...
    <v-dialog v-model="dialog">
      <v-card>
        ...
        <v-card-actions class="justify-end">
          ...
          <v-card-action>
            <v-btn block
              @click="async () => { await addTag({ id: 0, name: tagName }); await loadTags(); dialog = false; }">Save</v-btn>
          </v-card-action>
        </v-card-actions>
      </v-card>
    </v-dialog>
